...
<head>
    <title>BCU Event Portal - Delete Co-Ordinator</title>
    ...
    if (!isset($_SESSION['uid']) && ($_SESSION['utype'] == "dc")) {
        ...
        <form class="form-horizontal" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>"
            ...
            <div class="container mt-3 w-50 p-4 border border-primary rounded-3 shadow mb-5 bg-body-tertiary rounded">
                <h2 class="text-center text-primary">Delete Co-Ordinator</h2>
                <hr>
...
if (isset($_POST['delete'])) {
    ...
    if (mysqli_num_rows($checkResult) > 0) {
        $deleteSql = "DELETE FROM event_coordinator WHERE coor_id='$coordinatorId'";
        $deleteResult = mysqli_query($con, $deleteSql);

        if ($deleteResult) {
            echo ("<script>alert(\"Event Co-ordinator Deleted Successfully...\")</script>");
        } else {
