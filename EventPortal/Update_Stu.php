...
<head>
    <title>BCU Event Portal - Update Participant</title>
...
<body>
    ...
    if (!isset($_SESSION['uid']) && ($_SESSION['utype'] == "dc")) {
        ...
        <form class="form-horizontal" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>"
            ...
            <div class="container w-75 mt-3 p-4 border border-primary rounded-3 shadow mb-5 bg-body-tertiary rounded">
                <h2 class="text-center text-primary">Update Participant</h2>
                <hr>
    ...
    if (isset($_POST['update'])) {
        ...
        if (mysqli_num_rows($checkResult) > 0) {
            ...
            $course = $_POST['course'];
            $sql = "UPDATE student SET reg_no='$stu_id',name='$name', email='$email', phone='$phone',college='$collage', cource='$course' WHERE sid='$s_id'";
            $result = mysqli_query($con, $sql);
            if ($result) {
                echo ("<script>alert(\"Participant Updated Successfully...!\")</script>");
            } else {
                echo ("<script>alert(\"Participant Update Failed...\n\n Try Again\")</script>");
            }
        } else {
